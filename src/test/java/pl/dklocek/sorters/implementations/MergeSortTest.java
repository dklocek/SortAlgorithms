...
public class MergeSortTest {
    ...
    public void testSortStudent() throws Exception {
        ...
        Assert.assertArrayEquals(sorted, MergeSort.sortByName(testArray, false).get(0));

        sorted = testArray.clone();
        Arrays.sort(sorted, Student::compareBySurname);
        Assert.assertArrayEquals(sorted,MergeSort.sortBySurname(testArray,false).get(0));

        sorted = testArray.clone();
        Arrays.sort(sorted, Student::compareById);
        Assert.assertArrayEquals(sorted,MergeSort.sortById(testArray,false).get(0));

        sorted = testArray.clone();
        Arrays.sort(sorted, Student::compareByAge);
        Assert.assertArrayEquals(sorted,MergeSort.sortByAge(testArray,false).get(0));
    }
}